...
| `clip-dirty.glb` | The same clip with two deliberate, repairable defects: one non-unit rotation key and one sign-flipped key. Everything else is identical, so `fix` restores the rotations losslessly and `diff` reports no measurement drift against `clip.glb`. The repaired bytes are not `clip.glb`'s: `fix` is byte-surgical, and it patches the defective keys rather than reproducing another export. | `quat-norm`, `quat-flip` | none |
