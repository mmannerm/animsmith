...
jq -e --arg expected 'urn:animsmith:schema:measurements:15' '
  .["$defs"].measure_file_report.properties.measurements["$ref"] == $expected
  and .["$defs"].lint_file_report.properties.measurements["$ref"] == $expected
' docs/schemas/output-v12.schema.json >/dev/null \
  || fail 'immutable output-v12 measure and lint files must retain measurements-v15'
jq -e --arg expected 'urn:animsmith:schema:measurements:16' '
  .["$defs"].measure_file_report.properties.measurements["$ref"] == $expected
  and .["$defs"].lint_file_report.properties.measurements["$ref"] == $expected
' docs/schemas/output-v13.schema.json >/dev/null \
  || fail 'output-v13 measure and lint files must reference measurements-v16'
jq -e --arg expected 'urn:animsmith:schema:measurements:16' '
  .["$defs"].measure_file_report.properties.measurements["$ref"] == $expected
  and .["$defs"].lint_file_report.properties.measurements["$ref"] == $expected
' docs/schemas/output-v14.schema.json >/dev/null \
  || fail 'output-v14 measure and lint files must reference measurements-v16'
if ! jq -e '
...
if ! cmp -s docs/schemas/output-v13.schema.json <(
  sed \
    -e 's/urn:animsmith:schema:output:12/urn:animsmith:schema:output:13/g' \
    -e 's/animsmith output v12/animsmith output v13/' \
    -e 's/"const": 12 }/"const": 13 }/' \
    -e 's/measurements v15/measurements v16/' \
    -e 's/urn:animsmith:schema:measurements:15/urn:animsmith:schema:measurements:16/g' \
    docs/schemas/output-v12.schema.json
); then
  fail 'output-v13 must differ from immutable output-v12 only by outer and nested measurement identities'
fi
if ! jq -e '
  .["$defs"].prediction_provenance.properties.schema.const
...
check_schema docs/schemas/foot-cycle-parameterization-v1.schema.json urn:animsmith:schema:foot-cycle-parameterization:1 crates/animsmith-core/src/foot_cycle.rs crates/animsmith/src/foot_cycle_parameterization.rs docs/collection-contracts.md DESIGN.md
check_schema docs/schemas/foot-cycle-member-evidence-v1.schema.json urn:animsmith:schema:foot-cycle-member-evidence:1 crates/animsmith/src/foot_cycle_producer.rs docs/output.md docs/collection-contracts.md DESIGN.md
check_schema docs/schemas/foot-cycle-aggregate-evidence-v1.schema.json urn:animsmith:schema:foot-cycle-aggregate-evidence:1 crates/animsmith/src/foot_cycle_producer.rs docs/output.md docs/collection-contracts.md DESIGN.md
check_schema docs/schemas/contact-transform-result-v1.schema.json urn:animsmith:schema:contact-transform-result:1 crates/animsmith-core/src/contact_transform.rs docs/output.md docs/collection-contracts.md DESIGN.md
...
' docs/schemas/collection-output-v4.schema.json >/dev/null \
  || fail 'immutable collection-output-v4 must retain output-v12 and measurements-v15 references'
jq -e '
  any(.. | objects | .["$ref"]?; . == "urn:animsmith:schema:output:13")
...
' docs/schemas/collection-output-v5.schema.json >/dev/null \
  || fail 'collection-output-v5 must reference output-v13 and measurements-v16'
jq -e '
  any(.. | objects | .["$ref"]?; . == "urn:animsmith:schema:output:14")
  and any(.. | objects | .["$ref"]?; . == "urn:animsmith:schema:measurements:16#/$defs/clip_measurements")
...
jq -e '
  any(.. | objects | .["$ref"]?; . == "urn:animsmith:schema:output:17")
  and any(.. | objects | .["$ref"]?; . == "urn:animsmith:schema:measurements:16#/$defs/clip_measurements")
' docs/schemas/collection-output-v9.schema.json >/dev/null \
  || fail 'collection-output-v9 must reference output-v17 and measurements-v16'
jq -e '
  any(.. | objects | .["$ref"]?; . == "urn:animsmith:schema:output:18")
...
check_schema docs/schemas/skeleton-compatibility-v1.schema.json urn:animsmith:schema:skeleton-compatibility:1 crates/animsmith/src/skeleton_compare.rs docs/cli.md docs/output.md DESIGN.md
