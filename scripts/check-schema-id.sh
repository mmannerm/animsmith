...
check_schema docs/schemas/measurements-v3.schema.json urn:animsmith:schema:measurements:3 crates/animsmith-core/src/contract.rs docs/output.md
...
# between its version and command. Also prove that nested measurements v3 in a
...
if [ -n "$modern_scanner_regression" ]; then
  fail "legacy-envelope scanner misclassified nested measurements v3"
fi
