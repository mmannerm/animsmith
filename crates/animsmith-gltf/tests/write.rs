...
fn glb_json(bytes: &[u8]) -> serde_json::Value {
    ...
    serde_json::from_slice(&bytes[20..20 + json_len]).expect("GLB JSON chunk")
}
...
#[test]
fn legacy_glb_keeps_the_skinned_holder_node_projection() {
    use animsmith_gltf::write::{
        GlbProjectionPolicyV1, GlbWriteLimits, preflight_glb_bytes, write_glb_bytes,
    };
    ...
    let bytes =
        write_glb_bytes(&doc, GlbProjectionPolicyV1::Legacy, &receipt).expect("legacy GLB bytes");

    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("legacy-skinned.glb");
    let summary = animsmith_gltf::write::write(&doc, &path).expect("legacy path writes");
    assert_eq!(std::fs::read(&path).unwrap(), bytes);
    assert_eq!(summary.nodes, 3);

    // This raw GLB oracle deliberately describes the established legacy
    // projection instead of reloading its normalized model: skinned geometry
    // hangs from a fresh holder node at the scene root, not from bone 0.
    ...
    assert!(json["nodes"][0].get("skin").is_none());
    assert_eq!(json["nodes"][2]["name"], "complete_mesh_skinned");
    assert_eq!(json["nodes"][2]["mesh"], 0);
    assert_eq!(json["nodes"][2]["skin"], 0);
    assert_eq!(json["scenes"][0]["nodes"], serde_json::json!([0, 2]));
}
...
#[test]
fn strict_work_limit_charges_textures_and_populated_source_sidecars() {
    use animsmith_gltf::{
        WriteError,
        write::{GlbProjectionPolicyV1, GlbWriteLimits, preflight_glb_bytes},
    };
    ...
    let mut doc = complete_strict_asset_doc();
    // These are partial source facts, so the strict projection may proceed,
    // but preflight still walks and must charge every sidecar domain.
    doc.assets
        .material_resources
        .materials
        .push(SourceMaterialAsset {
            material_index: 0,
            name: Some("source-material".into()),
    ...
    doc.assets
        .material_resources
        .textures
        .push(SourceTextureAsset {
            texture_index: 0,
            name: Some("source-texture".into()),
            image_index: 0,
        });
    doc.assets.material_resources.images.push(SourceImageAsset {
        image_index: 0,
        name: Some("source-image".into()),
        source_kind: ImageSourceKind::Embedded,
        declared_mime_type: Some("image/png".into()),
        detected_container: Some(ImageContainerFormat::Png),
        leading_magic_hex: None,
        inspection: SourceImageInspection::Available {
            width: 1,
            height: 1,
            channel_count: 4,
    ...
    // 159 includes four copied embedded texture bytes, the complete source
    // skeleton/skin sidecars, and the populated material-resource sidecars.
    // The exact boundary and its first excess catch any omitted charge.
    ...
    assert!(matches!(
        preflight_glb_bytes(
            &doc,
            GlbProjectionPolicyV1::StrictFootCycleV1,
            GlbWriteLimits { max_work: 158, ..exact }
        ),
        Err(WriteError::Refused(message)) if message.contains("work")
    ));
}
