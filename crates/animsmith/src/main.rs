...
enum Cmd {
    ...
    #[command(
        long_about = "Assemble one runtime GLB from an authoritative skinned base and animation takes supplied by FBX or glTF inputs. The versioned recipe owns exact mesh selection, skeleton remapping, clip windows and mechanical transforms. Recipe v6 can opt into rest/bind scale canonicalization for glTF/GLB and the narrow inventory-complete normalized/baked FBX subset with explicit source selectors and expected factor; it validates the base and every clip basis before copying keys, composes canonicalization, grounding, and node removal, then proves one final raw GLB rewrite. Asset-property refusals exit 1; under --format json they emit producer-refusal v1 on stdout without publishing either output. Recipe, path, I/O, and publication errors exit 2 with stderr only. Source extraction, project policy, and publication remain consumer responsibilities."
    )]
    #[cfg(feature = "fbx")]
    Assemble {
