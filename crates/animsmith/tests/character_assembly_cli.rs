...
fn accepts_time_window_and_gltf_clip_input_while_pruning_unselected_assets() {
    ...
    let discovered_name = inspect_text
        .lines()
        .find_map(|line| line.strip_prefix("  node \"")?.strip_suffix('"'))
    ...
    let recipe = success_recipe()
        ...
        .replacen(
            "mesh_instances = [\"tri\"]",
            &format!("mesh_instances = [{discovered_name:?}]"),
            1,
        )
    ...
    assert_eq!(
        ...
        serde_json::json!([discovered_name.as_str()]),
