...
#[test]
fn rest_bind_publishes_a_pair_whose_evidence_names_the_appendix_d5_policy() {
    ...
    assert_eq!(
        ...
        "the `appendix-d-v5` common-factor band plus its unit-scale postcondition"
    );

    assert_eq!(result["tolerance"]["policy_id"], "appendix-d-v5");
    // Published rather than implied, and retained under the v5 identity: a
...
fn a_document_whose_size_alone_exceeds_the_proof_sampling_budget_is_refused_at_the_proof() {
    ...
    //
    // Hand-computed against the `appendix-d-v5` budget of 400,000,000:
    //
