...
// The frame one side draws at the shared phase: the one a selected finding
// pinned, or the sampled frame that phase lands on in this side's own grid.
// Every panel asks this one question, so the pose pane, the trails and the
// gait of a side always draw the same frame — and the overlay, drawing both
// sides in one pane, draws exactly the two frames the two panes would.
function frameFor(name, phase) {
  return selectedFrames ? selectedFrames[name] : Math.round(phase * Math.max(0, data[name].clip.frames - 1));
}
...
//
// Two selections have nothing to overlay, and both suspend it rather than
// drawing something the label would then have to explain away: a loop-seam
// context is one side's own two endpoint frames rather than one shared phase,
// and a side whose sampled positions are all non-finite has no skeleton to
// draw at all. Suspended, the two panes are drawn exactly as they are with
// the overlay off, and the before pane's label says so.
const OVERLAY_LEGEND = "overlay · before solid blue, after dashed orange";
const OVERLAY_AFTER_DASH = [6, 4];
...
function overlaySuspension() {
  if (selectedContext && selectedContext.kind === "seam") return "overlay suspended while a loop-seam context is selected";
  if (!sidePoseBounds.before || !sidePoseBounds.after) return "overlay suspended: one side has no finite sampled position to draw";
  return null;
}
const overlayActive = () => Boolean(overlayToggle.checked) && !overlaySuspension();
...
function drawSide(name, palette, phase) {
  const side = data[name];
  const frame = frameFor(name, phase);
  const canvas = q(`${name}-gl`);
  const overlaid = overlayActive();
  // While the overlay is on, the after side's skeleton is drawn over the
  // before one and its own canvas is hidden, so there is nothing to draw
  // here — its frame still labels its own source time.
  if (!canvas || (overlaid && name === "after")) return frame;
  ...
  // Only the before pane carries the overlay, so only it reports that the
  // overlay a reader switched on is standing down for this selection.
  const suspended = name === "before" && overlayToggle.checked ? overlaySuspension() : null;
  ...
  if (!bounds) {
    ...
    q(`${name}-pose-context`).textContent = contextLabel(structural && structural.label, unavailable, suspended);
    return frame;
  }
  ...
  // judged frame of it, in the paint the caller names. The dash belongs to
  // the skeleton being drawn — the overlay's after pass is dashed — so it is
  // set here and cleared again, and whatever is drawn next is solid.
  const drawFrame = (poseSide, poseFrame, style) => {
    ...
    for (let bone = 0; bone < data.bones.length; bone++) {
      ...
      context.beginPath(); context.arc(...point, bone === style.subject ? 6 : style.radius, 0, Math.PI * 2); context.fill();
    }
    context.setLineDash([]);
  };
  const seam = selectedContext && selectedContext.name === name && selectedContext.kind === "seam" ? selectedContext.value : null;
  if (seam) {
    drawFrame(side, seam.first_frame, { stroke: palette.accent, fill: palette.accent, subject: seam.subject_bone, radius: 3 });
    drawFrame(side, seam.last_frame, { stroke: palette.warning, fill: palette.warning, subject: seam.subject_bone, radius: 3 });
    const exact = frameFinite(side, seam.first_frame) && frameFinite(side, seam.last_frame);
    q(`${name}-pose-context`).textContent = contextLabel(exact
      ...
      : `loop seam pose drawing incomplete: an endpoint contains non-finite positions; finding and coverage evidence remain listed · affected ${seam.subject_bone_name || "bone unavailable"}`, suspended);
  } else if (overlaid) {
    // Both skeletons, in the pane's own projection and at the frames the two
    // panes would have drawn: a selected finding's subject bone is marked on
    // its own side's skeleton alone, so highlighting one never implicates the
    // other.
    const afterFrame = frameFor("after", phase);
    drawFrame(data.before, frame, { stroke: palette.accent, fill: palette.accent, subject: highlight.before, radius: 3 });
    drawFrame(data.after, afterFrame, { stroke: palette.warning, fill: palette.warning, subject: highlight.after, radius: 2, dash: OVERLAY_AFTER_DASH });
    q(`${name}-pose-context`).textContent = contextLabel(
      structural && structural.label,
      OVERLAY_LEGEND,
      frameFinite(data.before, frame) && frameFinite(data.after, afterFrame) ? "exact judged pose-grid frames" : INCOMPLETE_FRAME,
    );
  } else {
    drawFrame(side, frame, { stroke: palette.muted, fill: palette.ink, subject: highlight[name], radius: 3 });
    const exact = frameFinite(side, frame);
    q(`${name}-pose-context`).textContent = contextLabel(
      structural && structural.label,
      // A structural label already speaks for the pane; the note beside it is
      // the caveat that the frame it drew is not exact.
      exact ? (structural ? null : "exact judged pose-grid frame") : INCOMPLETE_FRAME,
      suspended,
    );
  }
  return frame;
}
// The after pane while its skeleton is drawn over the before one: the canvas
// is hidden, and the pane's own label says where its pose went. Its heading,
// identity and evidence stay — and so does the rule the rest of the document
// follows, that a surface which is not drawn says so rather than standing as
// an empty box.
function showAfterPose(shown) {
  const canvas = q("after-gl");
  if (canvas) canvas.hidden = !shown;
  if (!shown) q("after-pose-context").textContent = "judged pose drawn over the before pane, as the dashed skeleton";
}
...
function update() {
  ...
  showAfterPose(!overlayActive());
  const beforeFrame = drawSide("before", palette, phase), afterFrame = drawSide("after", palette, phase);
