...
// The transport and the overlay all drive the sampled poses, so a document
// that carries none leaves every one of them disabled rather than offering a
// control that does nothing when it is used.
const overlayToggle = q("overlay");
if (posesOmitted) { q("scrub").disabled = true; overlayToggle.disabled = true; }
...
// draw the same two frames by construction rather than by agreement.
const frameFor = (name, phase) => (selectedFrames ? selectedFrames[name] : phaseFrame(name, phase));
// How the two inputs are told apart wherever they are drawn together: the
// before solid in its token, the after dashed, finer and translucent over it,
// so a repair that changed nothing still reads as two rather than as one.
// One table serves the root chart and the overlaid pose pane. `radius` is a
// chart's phase dot and `joint` a skeleton's joint, and the word a label uses
// for a token lives beside the token, so no legend can name a colour the
// drawing does not paint.
const TOKEN_COLOUR = { accent: "blue", warning: "orange" };
const SIDE_STYLE = {
  before: { token: "accent", dash: null, width: 3, opacity: 1, radius: 6, joint: 3 },
  after: { token: "warning", dash: [7, 5], width: 1.6, opacity: .7, radius: 3.5, joint: 2 },
};
// The overlay: the after skeleton drawn over the before one in the before
// pane, at the same two frames and through the same shared camera, so a
// repair reads as one picture instead of two. Off by default — the document
// is about its two panes, and this is a way of looking at them.
//
// Two selections have nothing to overlay and suspend it rather than drawing
// something a label would then have to explain away: a loop-seam context is
// one side's own two endpoint frames rather than one shared phase, and a side
// whose sampled positions are all non-finite has no skeleton to draw. Only
// the first needs saying — a side with no finite position already says so in
// its own pane, and repeating it in the other one would state one fact twice.
const OVERLAY_LEGEND = `overlay · before solid ${TOKEN_COLOUR[SIDE_STYLE.before.token]}, after dashed ${TOKEN_COLOUR[SIDE_STYLE.after.token]}`;
const OVERLAY_POINTER = "judged pose drawn over the before pane, as the dashed skeleton";
const INCOMPLETE_FRAME = "pose drawing incomplete: selected frame contains non-finite positions; findings and coverage remain listed";
const SUSPENSION_NOTE = { seam: "overlay suspended while a loop-seam context is selected" };
function overlaySuspension() {
  if (selectedContext && selectedContext.kind === "seam") return "seam";
  if (!sidePoseBounds.before || !sidePoseBounds.after) return "unavailable";
  return null;
}
// Resolved once per draw and handed down, so one press of the box cannot be
// read two ways inside one frame.
function overlayState() {
  const wanted = Boolean(overlayToggle.checked);
  const suspension = wanted ? overlaySuspension() : null;
  return { active: wanted && !suspension, suspension };
}
// A pane's label is what it can say about its own drawing, in the order a
// reader needs it; anything it has nothing to say about is left out.
const contextLabel = (...parts) => parts.filter(Boolean).join(" · ");
...
function drawSide(name, palette, frames, overlay) {
  const canvas = q(`${name}-gl`);
  if (!canvas) return;
  const side = data[name], frame = frames[name];
  const selection = selectedContext && selectedContext.name === name ? selectedContext : null;
  const structural = selection && selection.kind === "structural" ? selection.value : null;
  const seam = selection && selection.kind === "seam" ? selection.value : null;
  // Every pane writes its own label, drawn or not: a selected finding's
  // disclosure belongs to the side the finding was found on, whichever pane
  // draws that side's pose. A structural label already speaks for the pane,
  // so the exact-frame note is added only where it is the whole statement.
  // Only the before pane carries the overlay, so only it reports a
  // suspension.
  const say = (...parts) => {
    q(`${name}-pose-context`).textContent = contextLabel(
      structural && structural.label,
      ...parts,
      name === "before" ? SUSPENSION_NOTE[overlay.suspension] : null,
    );
  };
  // The after pane while its skeleton is drawn over the before one: `update`
  // hides its canvas, and its label says where its pose went rather than
  // leaving a heading over an empty box.
  if (overlay.active && name === "after") { say(OVERLAY_POINTER); return; }
  ...
  if (!sidePoseBounds[name]) {
    say("pose drawing unavailable: sampled X/Y positions are non-finite; findings and coverage remain listed");
    return;
  }
  ...
  // One skeleton, drawn through this pane's projection: any side's grid, any
  // judged frame of it, in the paint the caller names. Solid, with the
  // ordinary joint, unless the caller says otherwise.
  const drawFrame = (poseSide, poseFrame, style) => {
    context.setLineDash(style.dash || []);
    context.lineWidth = 2; context.strokeStyle = style.stroke;
    for (let bone = 0; bone < parents.length; bone++) if (parents[bone] >= 0) {
      const a = project(posePoint(poseSide, poseFrame, parents[bone])), b = project(posePoint(poseSide, poseFrame, bone));
    ...
    for (let bone = 0; bone < data.bones.length; bone++) {
      const point = project(posePoint(poseSide, poseFrame, bone));
      if (!finitePoint(point)) continue;
      context.fillStyle = bone === style.subject ? palette.error : style.fill;
      context.beginPath(); context.arc(...point, bone === style.subject ? 6 : style.joint || SIDE_STYLE.before.joint, 0, Math.PI * 2); context.fill();
    }
  };
  if (seam) {
    drawFrame(side, seam.first_frame, { stroke: palette.accent, fill: palette.accent, subject: seam.subject_bone });
    drawFrame(side, seam.last_frame, { stroke: palette.warning, fill: palette.warning, subject: seam.subject_bone });
    const exact = frameFinite(side, seam.first_frame) && frameFinite(side, seam.last_frame);
    say(exact
      ? `loop seam exact endpoint poses — first ${seam.first_s.toFixed(3)}s ${TOKEN_COLOUR.accent} · last ${seam.last_s.toFixed(3)}s ${TOKEN_COLOUR.warning} · affected ${seam.subject_bone_name || "bone unavailable"}`
    ...
    // Both skeletons, in this pane's projection and at the two frames every
    // other panel is drawing: a selected finding's subject bone is marked on
    // its own side's skeleton alone, so highlighting one never implicates the
    // other.
    const overlayStyle = (which) => ({
      stroke: palette[SIDE_STYLE[which].token],
      fill: palette[SIDE_STYLE[which].token],
      dash: SIDE_STYLE[which].dash,
      joint: SIDE_STYLE[which].joint,
      subject: highlight[which],
    });
    drawFrame(data.before, frames.before, overlayStyle("before"));
    drawFrame(data.after, frames.after, overlayStyle("after"));
    const exact = frameFinite(data.before, frames.before) && frameFinite(data.after, frames.after);
    say(OVERLAY_LEGEND, exact ? (structural ? null : "exact judged pose-grid frames") : INCOMPLETE_FRAME);
  } else {
    drawFrame(side, frame, { stroke: palette.muted, fill: palette.ink, subject: highlight[name] });
    const exact = frameFinite(side, frame);
    say(exact ? (structural ? null : "exact judged pose-grid frame") : INCOMPLETE_FRAME);
  }
}
// The after pane's canvas while its skeleton is drawn over the before one.
// Its heading, identity and evidence stay, and its own label — written by
// `drawSide` like every other pane's — says where its pose went.
function showAfterPose(shown) {
  const canvas = q("after-gl");
  if (canvas) canvas.hidden = !shown;
}
...
function drawRootComparison(palette, frames) {
  ...
  // smaller, so a coincident pair still reads as two. It is the same device,
  // from the same table, that the overlaid pose pane draws the two skeletons
  // with.
  const styleFor = (name) => Object.assign({}, SIDE_STYLE[name], { color: palette[SIDE_STYLE[name].token] });
  const identical = rootTracksIdentical();
  const place = markLanes(ROOT_PANEL);
  for (const name of ["before", "after"]) {
    const points = rootTrack(data[name]), style = styleFor(name);
    if (!points) continue;
    ...
    if (style.dash) path["stroke-dasharray"] = style.dash.join(" ");
  ...
  let legendX = 18;
  for (const name of ["before", "after"]) {
    const state = rootState(data[name]), label = `${name} root ${state}`;
    svg.append(svgElement("text", { x: legendX, y: 20, fill: state === "unavailable" ? palette.muted : palette[SIDE_STYLE[name].token] }, label));
...
function drawTrails(name, palette, frames) {
  ...
  for (const role of Object.keys(TRAIL_TOKENS)) {
    ...
    if (spec.dash) path["stroke-dasharray"] = spec.dash.join(" ");
    svg.append(svgElement("path", path));
    const frame = frames[name];
    const selected = map(points[frame]);
...
function drawGait(name, palette, frames) {
...
// The overlay is a way of looking at the same two frames, so switching it
// changes nothing else: the selection, the shared phase and playback all
// stand, and the next draw simply puts both skeletons in one pane.
