...
Measured RM speed spans are `crouch-combat-8-way` 0.730–0.732 m/s (ratio 0.997); `run-combat-8-way` 2.475–2.500 m/s (ratio 0.990); `walk-combat-8-way` 0.692–0.765 m/s (ratio 0.905). Preserve authored variation unless the project declares a normalization policy; diagonals and cardinals require the full blend test. Current in-place gait-phase spreads are large enough that `sync=not-evaluated` remains the correct contract. The current catalog also measures `forward-speed-alternatives`, `draw-combat-put-away`, `combo-alternatives`, and `single-attack-alternatives` as evaluator-defined hypotheses; they remain candidate groupings.
