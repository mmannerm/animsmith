# Animation pack evaluation: Mixamo Male Locomotion
...
|---|---|
| Walk and standard run are named and measured. Idle, jump and turns appear by filename; their controller roles remain unclassified. | Check the named states and their transitions in the target game before treating this as a complete controller. |

### Content gaps and unknowns

The legacy count of 16 motions has no verified mapping to these 18 files. Classify the other named motions before adding them to a controller.
...
Best fit is male locomotion prototyping where a developer can own movement, define thresholds, and test the exact target character. It is a poor fit for drop-in root-motion, network, motion-matching, layered-animation, or production-ready claims.
...
AnimSmith 0.14.0 — The current evaluation reran 996 inspect, measure, and lint commands across the collection. This constituent admitted 18/18 files; baseline and declared-contract findings were revalidated, and the selected controller set was proposed from current files. Its clip roles remain unverified.
...
The current tests covered 18 FBX source files individually. The legacy metadata lists 16 motions, but its clip-to-file mapping is unverified for the 18 files. The [evidence appendix](mixamo-male-locomotion-evidence.md) records the source integrity check, evaluator revision, command results, and digests. See the [canonical readiness ladder](../game-ready-clips.md#the-readiness-ladder).
