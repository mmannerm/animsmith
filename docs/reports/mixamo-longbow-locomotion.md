...
For a prototype, start with four-direction full-body bow running using the selected files in the [runtime set](#runtime-sets-and-authored-motion). Let the kinematic controller own XZ movement, yaw, and collision; the selected roots are effectively stationary. This is an evaluator-proposed set (`hypothesis/kinematic-run-4way`), with no accepted loop, blend, foot-contact, or target-engine result. Keep it out of production until the [issue gates](#technical-issue-register) and [engine review](#engine-status) pass.

All 22 delivered FBX files loaded under AnimSmith 0.14.0 and the untouched baseline produced 0 errors, 3 `duration-sanity` warnings, and 3126 non-gating `constant-track` notes. Applying one animation-owned movement rule across the archive produced 3 errors where files in the root-motion directory remain effectively stationary. Decide movement ownership per file before using those states; a directory name alone does not establish authored displacement. Two `X Bot.fbx` files use a 68-bone reference skeleton; the other 20 files use 66 bones, and every file resolves the nine-role Mixamo profile.
...
|---|---|
| Four standing run directions are named and measured. Standing idle, turn and forward-stop files appear by filename; their controller roles are unclassified and untested. | Check the named states and their transitions in the target game before treating this as a complete controller. |

### Content gaps and unknowns

The legacy count of 20 motions has no verified mapping to these 22 files. Classify the other named motions before adding them to a controller.
...
|---|---|---|---|
| `hypothesis/kinematic-run-4way` | Four-direction full-body bow run; controller owns XZ translation, yaw and collision | The four selected runs support a controller-owned movement prototype. Duration and phase differ between directions; inspect contacts before blending. Three idle/turn files need per-file ownership; the forward-stop file has an endpoint warning. No AnimSmith repair was tested; engine acceptance remains open. | [Member timings, coordinates and contracts](mixamo-longbow-locomotion-evidence.md#exact-runtime-members) |

The selected files are grouped by name and measurement. AnimSmith measured phase (cycles) as forward `0.4578`, back `0.5651`, left `0.6134`, and right `0.4589`; their measured phase spread is `0.1557` cycles. These phase numbers do not establish matching foot contacts or a usable blend. Loop intent, contact alignment, and gameplay use remain `not-evaluated`.

## Integration recipe

1. **Members/topology:** Use the exact members and coordinates for `hypothesis/kinematic-run-4way` in the linked appendix as a proposed directional blend. Add other directions or in-place/root-motion alternatives only after checking their own files.
...
AnimSmith 0.14.0 — The current evaluation reran 996 inspect, measure, and lint commands across the collection. This constituent admitted 22/22 files; baseline and declared-contract findings were revalidated, and the selected controller set was proposed from current files. Its clip roles remain unverified.
...
The current tests covered 22 FBX source files individually. The legacy metadata lists 20 motions, but its clip-to-file mapping is unverified for the 22 files. The [evidence appendix](mixamo-longbow-locomotion-evidence.md) records the source integrity check, evaluator revision, command results, and digests. See the [canonical readiness ladder](../game-ready-clips.md#the-readiness-ladder).
