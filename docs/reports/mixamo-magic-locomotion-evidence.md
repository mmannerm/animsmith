# Animation pack evidence appendix: Mixamo Magic Locomotion

> Companion report: [Technical report](mixamo-magic-locomotion.md)
>
...
| Field | Value |
|---|---|
| Pack/edition | Mixamo Magic Locomotion; local revision unknown |
| Vendor/source | Vendor identity observed in delivered metadata; listing URL not retained |
| Delivered scope | 27 extracted FBX files: 10 in-place-directory and 17 root-motion-directory files |
...
| Evaluation manifest | `mixamo-magic-locomotion.manifest.json`; SHA-256 `c766dea7a1f0e34d83fcf7f4686f9540f72b514643e28038b81e9ea9a1c8909d`; legacy schema `urn:animsmith:skill:animation-pack-evaluation-manifest:1` |
...
The legacy metadata count of 25 motions is not linked authoritatively to these 27 files. Current conclusions use exact source paths and fresh measurements only.

### Evidence coverage

| Surface | Offered/delivered | Evaluated | Findings | Not evaluated and why |
|---|---:|---:|---:|---|
| Animation/source files | 27 | 27 | 0 baseline errors; 6 warnings | Vendor motion mapping unavailable |
| Rigs/export variants | 2 skeleton counts | 27 | two 68-bone `X Bot.fbx`; 25 66-bone files; Mixamo roles resolved | Retarget/deformation unavailable |
| AnimSmith baseline | 27 | 27 | 6 warnings; 3837 notes | — |
| Declared contracts | 27 | 27 | 4 ownership errors; 6 warnings | Per-file project intent unavailable |
...
|---|---|---|---|---|
| `hypothesis/kinematic-run-4way` | directional-blend | `Magic_Locomotion_Pack_-_in-place/Standing Run Forward.fbx`; `Magic_Locomotion_Pack_-_in-place/Standing Run Back.fbx`; `Magic_Locomotion_Pack_-_in-place/Standing Run Left.fbx`; `Magic_Locomotion_Pack_-_in-place/Standing Run Right.fbx`; all `in-place` | new evaluator hypothesis from current source names; no vendor/runtime authority; exact durations and frames are in the primary table | `not-evaluated` runtime acceptance |
...
| Inspect | `evaluated-clean` | All 27 FBXs admitted and resolved the Mixamo profile |
...
|---|---|---|---|
| Opaque delivered corpus | 0 errors; 6 warnings; all sources readable | Mixamo roles resolved; vendor semantics and retarget use unavailable | No current engine, visual, contact, or gameplay acceptance |
...
The fresh inventory contains 27 FBX files. Both directory variants include `X Bot.fbx` with a 68-bone skeleton and an empty `Take 001`; the 25 motion-named files use 66 bones. All sources resolved the same nine Mixamo roles. This establishes static structure, not identical hierarchy/rest pose or retarget compatibility. Constant-track findings are optimization hints only.

The current scenario is intended to be useful without claiming old authority: `hypothesis/kinematic-run-4way` names 4 exact in-place-directory files, records current durations/frame counts, assigns movement to a hypothetical kinematic controller, and leaves loop intent, gait-phase acceptance, transitions, and runtime acceptance open. AnimSmith measured phase (cycles) as forward `0.4370`, back `0.6134`, left `0.4670`, and right `0.4208`; the normalized circular-range spread is `0.1926` cycles, defined as `1 - largest cyclic gap` across member phases on the unit cycle. These are `observed-animsmith` measurements; `sync=not-evaluated`, and no defect, contact acceptance, or synchronization acceptance follows from the spread alone.

## Mechanical baseline

| Finding/check | Affected scope | Potential impact | Evidence |
|---|---|---|---|
| No baseline errors | 27/27 files | File-readable mechanical prerequisite only | `observed-animsmith`; current ledger |
| `duration-sanity` | 6 findings in 6 files | Empty reference take or shorter channels clamp-held at clip end | `Magic_Locomotion_Pack_-_in-place/X Bot.fbx`; `Magic_Locomotion_Pack_-_root-motion/Standing Jump.fbx`; `Magic_Locomotion_Pack_-_root-motion/Standing Land To Standing Idle.fbx`; `Magic_Locomotion_Pack_-_root-motion/Standing Turn Left 90.fbx`; `Magic_Locomotion_Pack_-_root-motion/X Bot.fbx`; `Magic_Locomotion_Pack_-_root-motion/standing idle.fbx` |
| `constant-track` | 3837 notes | Possible storage/evaluation overhead; no visual defect or safe rewrite inferred | `observed-animsmith` |
| Mixamo role resolution | 27/27 files | Enables measurement; does not prove target-character retargeting | `observed-animsmith` |

## AnimSmith remediation evidence

| Source issue | Operation/declarations | Result | Independent verification | Remaining caveat |
|---|---|---|---|---|
| MIX-OWN-001 | Archive-level in-place and animation-owned XZ declarations | 4 stationary-root errors on `Magic_Locomotion_Pack_-_root-motion/Standing Jump.fbx`; `Magic_Locomotion_Pack_-_root-motion/Standing Turn Left 90.fbx`; `Magic_Locomotion_Pack_-_root-motion/Standing Turn Right 90.fbx`; `Magic_Locomotion_Pack_-_root-motion/standing idle.fbx` | Current per-file lint and measurement; no output produced | Per-file intent and controller acceptance unavailable; residual unresolved |
...
1. Vendor mapping from 25 metadata motions to 27 files is unavailable and was not reconstructed from names.
...
AnimSmith 0.14.0 — Fresh current-source matrix admitted 27/27 FBXs and revalidated baseline and archive-level declarations. New controller topology is explicitly evaluator-proposed and remains runtime-unaccepted.
