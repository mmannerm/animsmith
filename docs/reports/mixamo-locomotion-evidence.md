# Animation pack evidence appendix: Mixamo Locomotion

> Companion report: [Technical report](mixamo-locomotion.md)
>
...
| Field | Value |
|---|---|
| Pack/edition | Mixamo Locomotion; local revision unknown |
...
| Evaluation manifest | `mixamo-locomotion.manifest.json`; SHA-256 `b9478ef47ba6fbf158737c867a2e7e6a253c910c48b0b3c0a77f133950f005e9`; legacy schema `urn:animsmith:skill:animation-pack-evaluation-manifest:1` |
...
| AnimSmith baseline | 20 | 20 | 5 warnings; 2852 notes | — |
...
|---|---|---|---|---|---|
| `hypothesis/kinematic-speed` | walk | `Locomotion_Pack_-_in-place/walking.fbx` | variant=in-place | duration=1.033 s; frames=32 frames | loop=unknown; sync=not-evaluated; movement=controller-xz-yaw; playback=project-defined |
| `hypothesis/kinematic-speed` | run | `Locomotion_Pack_-_in-place/running.fbx` | variant=in-place | duration=0.700 s; frames=22 frames | loop=unknown; sync=not-evaluated; movement=controller-xz-yaw; playback=project-defined |
...
|---|---|---|---|---|
| `hypothesis/kinematic-speed` | speed-blend | `Locomotion_Pack_-_in-place/walking.fbx`; `Locomotion_Pack_-_in-place/running.fbx`; all `in-place` | new evaluator hypothesis from current source names; no vendor/runtime authority; exact durations and frames are in the primary table | `not-evaluated` runtime acceptance |
...
The current scenario is intended to be useful without claiming old authority: `hypothesis/kinematic-speed` names 2 exact in-place-directory files, records current durations/frame counts, assigns movement to a hypothetical kinematic controller, and leaves loop intent, gait-phase acceptance, transitions, and runtime acceptance open. AnimSmith measured phase (cycles) as walk `0.4841` and run `0.4529`; the normalized circular-range spread is `0.0312` cycles, defined as `1 - largest cyclic gap` across member phases on the unit cycle. These are `observed-animsmith` measurements; `sync=not-evaluated`, and no contact or synchronization acceptance follows from the spread.
...
| `duration-sanity` | 5 findings in 5 files | Empty reference take or shorter channels clamp-held at clip end | `Locomotion_Pack_-_in-place/X Bot.fbx`; `Locomotion_Pack_-_root-motion/X Bot.fbx`; `Locomotion_Pack_-_root-motion/idle.fbx`; `Locomotion_Pack_-_root-motion/left turn 90.fbx`; `Locomotion_Pack_-_root-motion/right strafe.fbx` |
| `constant-track` | 2852 notes | Possible storage/evaluation overhead; no visual defect or safe rewrite inferred | `observed-animsmith` |
