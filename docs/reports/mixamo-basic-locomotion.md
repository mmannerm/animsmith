...
For a prototype, start with three-direction unarmed walking using the selected files in the [runtime set](#runtime-sets-and-authored-motion). Let the kinematic controller own XZ movement, yaw, and collision; the selected roots are effectively stationary. This is an evaluator-proposed set (`hypothesis/kinematic-walk-3way`), with no accepted loop, blend, foot-contact, or target-engine result. Keep it out of production until the [issue gates](#technical-issue-register) and [engine review](#engine-status) pass.

All 12 delivered FBX files loaded under AnimSmith 0.14.0 and the untouched baseline produced 0 errors, 4 `duration-sanity` warnings, and 1712 non-gating `constant-track` notes. Applying one animation-owned movement rule across the archive produced 4 errors where files in the root-motion directory remain effectively stationary. Decide movement ownership per file before using those states; a directory name alone does not establish authored displacement. Two `X Bot.fbx` files use a 68-bone reference skeleton; the other 10 files use 66 bones, and every file resolves the nine-role Mixamo profile.
