...
| Delivered scope | 22 extracted FBX files from paired archive variants |
...
| Evaluation manifest | `mixamo-longbow-locomotion.manifest.json`; SHA-256 `39e8baadfd586d57d71d4e5831051d9cb3036af4540b8c8bc7e7646e111c1b5f`; schema `urn:animsmith:skill:animation-pack-evaluation-manifest:1` |
| Acquisition/license provenance | No controlling license/readme/terms evidence was delivered; not legal advice |

The evaluation manifest schema is `urn:animsmith:skill:animation-pack-evaluation-manifest:1`. The validated manifest models every delivered FBX as an opaque `other-unknown` unit because vendor records were not linked to archive members; 20 remains separate source metadata.

### Evidence coverage

| Surface | Offered/delivered | Evaluated | Findings | Not evaluated and why |
|---|---:|---:|---:|---|
| Animation files | 22 | 22 | 0 errors; 3 baseline warnings | Semantic file-to-manifest mapping incomplete |
| Rigs/export variants | 1 | 1 | Mixamo profile used | Retarget behavior unavailable |
| AnimSmith baseline | 22 | 22 | 0 errors; 3 warnings; 3,126 notes | — |
| Declared contracts | 22 | 22 | 3 root-motion stationary-root errors; 1 in-place and 2 root-motion duration warnings | Per-clip intent unavailable |
...
| `other-unknown` | 22 | 22 | One opaque unit per delivered FBX; no semantic classification asserted |
| **Total** | **22** | **22** | Validated scrubbed external manifest |
...
| Root motion | `partially-evaluated` | In-place XZ contract has 0 errors and 1 duration warning; root-motion XZ contract has 3 stationary-root errors and 2 duration warnings; per-clip ownership remains open |
...
The external inventory contains 22 FBX files and 20 manifest-declared motions. Archive member counts exceed manifest motion counts by one non-motion/reference asset per archive. Delivered metadata declares 30 fps FBX 2019 export preferences, but this is not a target-engine import result.

## Mechanical baseline

| Finding/check | Affected scope | Potential impact | Evidence |
|---|---|---|---|
| No mechanical errors | 22 files | File-ready mechanical health only | observed-animsmith, AnimSmith 0.10.0 |
| `duration-sanity` warnings | 3 findings | Two no-track clips and one channel-end mismatch require semantic classification or review | observed-animsmith, AnimSmith 0.10.0 |
...
AnimSmith 0.10.0 — Official `v0.10.0` rerun revalidated output schema v19 and measurements schema v18: 22/22 FBX admitted, baseline 0 errors, and the root-motion declaration found 3 stationary-root errors. AnimSmith 0.7.0 (output schema v17; measurements schema v16) is retained historical evidence only.
