# Animation pack evidence appendix: Mixamo Longbow Locomotion

> Companion report: [Technical report](mixamo-longbow-locomotion.md)
>
...
| Field | Value |
|---|---|
| Pack/edition | Mixamo Longbow Locomotion; local revision unknown |
| Vendor/source | Vendor identity observed in delivered metadata; listing URL not retained |
| Delivered scope | 22 extracted FBX files: 9 in-place-directory and 13 root-motion-directory files |
...
| Evaluation manifest | `mixamo-longbow-locomotion.manifest.json`; SHA-256 `317fa928cb79eef6dd447e936438a9114100d47ae6b8191af1dd53111e137336`; legacy schema `urn:animsmith:skill:animation-pack-evaluation-manifest:1` |
...
The legacy metadata count of 20 motions is not linked authoritatively to these 22 files. Current conclusions use exact source paths and fresh measurements only.

### Evidence coverage

| Surface | Offered/delivered | Evaluated | Findings | Not evaluated and why |
|---|---:|---:|---:|---|
| Animation/source files | 22 | 22 | 0 baseline errors; 3 warnings | Vendor motion mapping unavailable |
| Rigs/export variants | 2 skeleton counts | 22 | two 68-bone `X Bot.fbx`; 20 66-bone files; Mixamo roles resolved | Retarget/deformation unavailable |
| AnimSmith baseline | 22 | 22 | 3 warnings; 3126 notes | — |
| Declared contracts | 22 | 22 | 3 ownership errors; 3 warnings | Per-file project intent unavailable |
| Evaluator scenario | 1 | 1 static proposal | 4 members with measured gait phase | Loop, synchronization, contact, engine, and visual acceptance unavailable |
...
| `other-unknown` | 22 | 22 | One opaque current source unit per delivered FBX |
| **Total** | **22** | **22** | Current exact source paths; no vendor mapping inferred |

### Exact runtime members

| Set/profile | Role or coordinate | Exact members | Variant/type | Timing or motion | Runtime contract |
|---|---|---|---|---|---|
| `hypothesis/kinematic-run-4way` | forward=(0,1) | `Longbow_Locomotion_Pack_-_in-place/standing run forward.fbx` | variant=in-place | duration=0.867 s; frames=27 frames | loop=unknown; sync=not-evaluated; movement=controller-xz-yaw; playback=project-defined |
| `hypothesis/kinematic-run-4way` | back=(0,-1) | `Longbow_Locomotion_Pack_-_in-place/standing run back.fbx` | variant=in-place | duration=0.667 s; frames=21 frames | loop=unknown; sync=not-evaluated; movement=controller-xz-yaw; playback=project-defined |
| `hypothesis/kinematic-run-4way` | left=(-1,0) | `Longbow_Locomotion_Pack_-_in-place/standing run left.fbx` | variant=in-place | duration=0.667 s; frames=21 frames | loop=unknown; sync=not-evaluated; movement=controller-xz-yaw; playback=project-defined |
| `hypothesis/kinematic-run-4way` | right=(1,0) | `Longbow_Locomotion_Pack_-_in-place/standing run right.fbx` | variant=in-place | duration=0.767 s; frames=24 frames | loop=unknown; sync=not-evaluated; movement=controller-xz-yaw; playback=project-defined |
...
|---|---|---|---|---|
| `hypothesis/kinematic-run-4way` | directional-blend | `Longbow_Locomotion_Pack_-_in-place/standing run forward.fbx`; `Longbow_Locomotion_Pack_-_in-place/standing run back.fbx`; `Longbow_Locomotion_Pack_-_in-place/standing run left.fbx`; `Longbow_Locomotion_Pack_-_in-place/standing run right.fbx`; all `in-place` | new evaluator hypothesis from current source names; no vendor/runtime authority; exact durations and frames are in the primary table | `not-evaluated` runtime acceptance |
...
| Inspect | `evaluated-clean` | All 22 FBXs admitted and resolved the Mixamo profile |
...
|---|---|---|---|
| Opaque delivered corpus | 0 errors; 3 warnings; all sources readable | Mixamo roles resolved; vendor semantics and retarget use unavailable | No current engine, visual, contact, or gameplay acceptance |
| `hypothesis/kinematic-run-4way` | Exact members and gait phases measured; loop intent unknown | New evaluator hypothesis; controller ownership proposed | Target-engine blend, phase/contact acceptance, transition, and deformation test required |
...
| Root-motion controller | `selected` — `observed-pack-capability` | 3 archive-level ownership conflicts require per-file policy |
...
The fresh inventory contains 22 FBX files. Both directory variants include `X Bot.fbx` with a 68-bone skeleton and an empty `Take 001`; the 20 motion-named files use 66 bones. All sources resolved the same nine Mixamo roles. This establishes static structure, not identical hierarchy/rest pose or retarget compatibility. Constant-track findings are optimization hints only.

The current scenario is intended to be useful without claiming old authority: `hypothesis/kinematic-run-4way` names 4 exact in-place-directory files, records current durations/frame counts, assigns movement to a hypothetical kinematic controller, and leaves loop intent, gait-phase acceptance, transitions, and runtime acceptance open. AnimSmith measured phase (cycles) as forward `0.4578`, back `0.5651`, left `0.6134`, and right `0.4589`; the normalized circular-range spread is `0.1557` cycles, defined as `1 - largest cyclic gap` across member phases on the unit cycle. These are `observed-animsmith` measurements; `sync=not-evaluated`, and no defect, contact acceptance, or synchronization acceptance follows from the spread alone.

## Mechanical baseline

| Finding/check | Affected scope | Potential impact | Evidence |
|---|---|---|---|
| No baseline errors | 22/22 files | File-readable mechanical prerequisite only | `observed-animsmith`; current ledger |
| `duration-sanity` | 3 findings in 3 files | Empty reference take or shorter channels clamp-held at clip end | `Longbow_Locomotion_Pack_-_in-place/X Bot.fbx`; `Longbow_Locomotion_Pack_-_root-motion/X Bot.fbx`; `Longbow_Locomotion_Pack_-_root-motion/standing run forward stop.fbx` |
| `constant-track` | 3126 notes | Possible storage/evaluation overhead; no visual defect or safe rewrite inferred | `observed-animsmith` |
| Mixamo role resolution | 22/22 files | Enables measurement; does not prove target-character retargeting | `observed-animsmith` |

## AnimSmith remediation evidence

| Source issue | Operation/declarations | Result | Independent verification | Remaining caveat |
|---|---|---|---|---|
| MIX-OWN-001 | Archive-level in-place and animation-owned XZ declarations | 3 stationary-root errors on `Longbow_Locomotion_Pack_-_root-motion/standing idle 01.fbx`; `Longbow_Locomotion_Pack_-_root-motion/standing turn 90 left.fbx`; `Longbow_Locomotion_Pack_-_root-motion/standing turn 90 right.fbx` | Current per-file lint and measurement; no output produced | Per-file intent and controller acceptance unavailable; residual unresolved |
...
|---|---|---|---|---|---|
| `hypothesis/kinematic-run-4way` / target character | Mixamo roles resolve; target deformation not tested | Not evaluated | Controller XZ/yaw/collision proposed | Exact durations and gait phases measured; synchronization/blend acceptance not tested | `not-evaluated` beyond static source evidence |
...
1. Vendor mapping from 20 metadata motions to 22 files is unavailable and was not reconstructed from names.
...
AnimSmith 0.14.0 — Fresh current-source matrix admitted 22/22 FBXs and revalidated baseline and archive-level declarations. New controller topology is explicitly evaluator-proposed and remains runtime-unaccepted.
