# Animation pack evidence appendix: Protofactor Ultimate Animation Collection (partial: eight packs)

> Companion report: [Protofactor Ultimate Animation Collection report](protofactor-ultimate-animation-collection.md)
>
> Evidence status: **partial** — current official 0.10.0 evidence is constituent-derived; collection binding, runtime, engine, and visual evidence are absent.
>
> Evaluation date: **2026-09-01**
>
> Current evaluator: **AnimSmith 0.10.0**
>
> Report format: **2**

This appendix preserves current constituent evidence and distinctly labeled retained dated observations. The [canonical readiness ladder](../game-ready-clips.md#the-readiness-ladder) remains authoritative.

Evaluation manifest schema: `urn:animsmith:skill:animation-pack-evaluation-manifest:1`.

## Evaluation scope and provenance

| Field | Value |
|---|---|
| Pack/edition | Protofactor Ultimate Animation Collection; partial evaluation of eight constituents |
| Vendor/source | [Protofactor Ultimate Animation Collection](https://protofactor.biz/product/ultimate-animation-collection/) |
| Included scope | Basic Locomotion, Sword & Shield, Campfire, Climbing, Injured, 1-Handed Melee, 2-Handed Melee, and Dual Swords |
| Excluded scope | Fifteen named constituents; no current inventory, classification, compatibility, or readiness conclusion for them |
| Delivered scope | Authorized local commercial deliveries; 918 source candidates, including 895 individual motion-labelled inputs |
| Target use | Engine-neutral, mechanically scoped collection intake |
| Target engines | No current engine run; a dated Unity 6000.5.8f1 observation is retained separately |
| Source manifests | External scrubbed constituent inventories; digests and commands are recorded in the eight linked appendices |
| Evaluation manifest | Current collection binding unavailable; no current collection output was rendered or accepted |
| Acquisition/license provenance | Commercial source was authorized locally; no transaction, redistribution, or licensing conclusion is made |

### Evidence coverage

| Surface | Offered/delivered | Evaluated | Findings | Not evaluated and why |
|---|---:|---:|---:|---|
| Source candidates | 918 | 918 | constituent baseline completed | Current mechanical evidence is constituent-derived |
| Individual motion-labelled inputs | 895 | 895 | constituent declared-contract results recorded | No collection semantic classification was accepted |
| Canonical logical motions | unknown | 0 | 0 | No current collection binding or taxonomy output |
| Runtime sets | unknown | 0 | 0 | No current runtime grouping was selected |
| AnimSmith baseline | 918 | 918 | 3,672 commands completed | Per-constituent findings are linked below |
| AnimSmith transform candidates | 103 | 103 | External outputs from four melee constituents | No promotion, engine, visual, or artistic acceptance |
| Shared package paths | 700 retained dated comparisons | 700 retained dated comparisons | byte-identical; zero conflicts | Not rerun in the current refresh |
| Engine import/playback | 4 runtimes | 0 current | 0 current | No current engine run selected |

### Claim legend

Current consequential claims use `observed-file`, `observed-animsmith`, `documentation-stated`, or `not-evaluated` as defined by the assessment taxonomy. Retained dated shared-path and engine observations are not current evaluator evidence.

## Evaluation manifest and taxonomy

### Canonical clip-role inventory

| Canonical primary role | Logical motions | Delivered files | Evidence boundary |
|---|---:|---:|---|
| `idle-pose` | 0 | 0 | Classification was not regenerated or accepted |
| `continuous-locomotion` | 0 | 0 | Classification was not regenerated or accepted |
| `locomotion-transition` | 0 | 0 | Classification was not regenerated or accepted |
| `airborne` | 0 | 0 | Classification was not regenerated or accepted |
| `traversal` | 0 | 0 | Classification was not regenerated or accepted |
| `action-interaction` | 0 | 0 | Classification was not regenerated or accepted |
| `reaction-death` | 0 | 0 | Classification was not regenerated or accepted |
| `emote-cinematic` | 0 | 0 | Classification was not regenerated or accepted |
| `other-unknown` | 0 | 0 | Classification was not regenerated or accepted |
| **Total** | **0** | **0** | 918 source candidates and 895 motion-labelled inputs remain unclassified at collection level |

### Runtime-set inventory

No runtime sets were identified.

### Pipeline-stage coverage

| Stage | Coverage state | Evidence / remaining gate |
|---|---|---|
| Acquire | `partially-evaluated` | Authorized local inventories for eight constituents only |
| Preserve raw | `evaluated-clean` | Commercial source was not modified |
| Inspect | `evaluated-finding` | Current constituent baseline completed for all source candidates |
| Segment | `not-evaluated` | No collection segmentation trial selected |
| Root motion | `not-evaluated` | No collection root-motion policy or controller acceptance |
| Conform | `not-evaluated` | No target rig, retarget trial, or conformance policy |
| Validate | `evaluated-finding` | Current constituent declared contracts recorded |
| Optimize | `partially-evaluated` | 103 external transform candidates; no collection promotion or acceptance |
| Export | `not-evaluated` | No runtime handoff selected |
| Gate/report | `partially-evaluated` | Current constituent-derived boundary recorded |

### Readiness evidence by clip set

| Role or runtime set | File-ready / clip-ready | Set-ready / rig-use | Runtime / acceptance boundary |
|---|---|---|---|
| Eight-constituent source corpus | evaluated-finding | not-evaluated | not-evaluated |
| Current collection roles or runtime sets | not-evaluated | not-evaluated | not-evaluated |

### Validation-profile status

| Validation profile | Selection | Result / next evidence |
|---|---|---|
| Marketplace intake | `selected` — `evaluator-selected-generic-scenario` | Current source inventory and constituent baseline captured |
| Blended locomotion | `not-selected` | Select clips and run current cross-pack blend evidence |
| Root-motion controller | `not-selected` | Declare root ownership and run a controller test |
| State-machine transitions | `not-selected` | Declare handoffs and run selected transition evidence |
| Layered upper body/weapons | `not-selected` | Define masks and test selected clips |
| Traversal/environment | `not-selected` | Define environment/contact decisions |
| Contact actions/interactions | `not-selected` | Define prop/contact decisions |
| Retargeted/customizable characters | `not-selected` | Supply a target rig and retarget test |
| Motion matching/search | `not-selected` | Supply a database and selection policy |
| Networked movement | `not-selected` | Supply an authority/rollback policy |
| Runtime performance | `not-selected` | Supply a runtime build and budget |

## Pack inventory and content evidence

| Constituent | Source candidates | Individual motion-labelled inputs | Current evidence |
|---|---:|---:|---|
| Basic Locomotion | 179 | 177 | baseline and declared contracts |
| Sword & Shield | 136 | 132 | baseline, declared contracts, and 28 external transform candidates |
| Campfire | 29 | 25 | baseline and declared contracts |
| Climbing | 77 | 75 | baseline and declared contracts |
| Injured | 72 | 70 | baseline and declared contracts |
| 1-Handed Melee | 113 | 110 | baseline, declared contracts, and 25 external transform candidates |
| 2-Handed Melee | 123 | 120 | baseline, declared contracts, and 25 external transform candidates |
| Dual Swords | 189 | 186 | baseline, declared contracts, and 25 external transform candidates |
| **Total** | **918** | **895** | Current evidence is constituent-derived |

Names and packaging are not current semantic role proof. The fifteen excluded constituents are outside this inventory and were not used to infer collection capability.

## Mechanical baseline

| Constituent | Current baseline or declared-contract finding | Evidence boundary |
|---|---|---|
| Basic Locomotion | 24,186 `constant-track` notes and 36 `time-monotonic` errors across 12 files; 58 pass and 119 fail per output format | `observed-animsmith` |
| Sword & Shield | 544 baseline commands; 34 clean and 230 non-clean declared-contract results | `observed-animsmith` |
| Campfire | 3,664 `constant-track` notes across 27 animated files; 17 pass and 8 fail per output format | `observed-animsmith` |
| Climbing | 9,011 `constant-track` notes; 34 pass and 41 fail per output format | `observed-animsmith` |
| Injured | 9,915 `constant-track` notes; 28 pass and 42 fail per output format | `observed-animsmith` |
| 1-Handed Melee | 452 baseline commands; 23 clean and 87 non-clean declared-contract results per output format | `observed-animsmith` |
| 2-Handed Melee | 492 baseline commands and one duration warning; 13 clean and 107 non-clean declared-contract results per output format | `observed-animsmith` |
| Dual Swords | 756 baseline commands; 24 clean and 162 non-clean declared-contract results per output format | `observed-animsmith` |

The aggregate baseline count is 3,672 commands. These are mechanical diagnostics, not an engine or artistic acceptance result.

## AnimSmith remediation evidence

| Scope | Operation/declarations | Result | Independent verification | Remaining caveat |
|---|---|---|---|---|
| Collection | No collection transform or remediation run | No current collection output or promoted derivative | None; no collection binding was rendered | Do not infer cross-pack use from constituent candidates |
| Sword & Shield | Bounded external transform declarations | 28 candidates; commands exited successfully | Constituents retain their exact command records | Candidate quality and engine use remain unaccepted |
| 1-Handed, 2-Handed, and Dual Swords | Bounded external transform declarations | 25 candidates per constituent; commands exited successfully | Constituents retain their exact command records | Candidate quality and engine use remain unaccepted |

No commercial source or derivative is published by this report.

## Engine procedures and evidence

| Runtime | Procedure/evidence | Result | Boundary |
|---|---|---|---|
| Unity 6000.5.8f1 | Retained dated contextual and melee graph probes | 22/22 and 33/33 required checks recorded; four expected Generic-rig failures isolated | Not rerun in the current refresh; no current engine, visual, artistic, or gameplay acceptance |
| Unreal Engine | No current procedure selected | not-evaluated | Import, retarget, graphs, contacts, and build remain |
| Godot | No current procedure selected | not-evaluated | Conversion/import, graphs, contacts, and export remain |
| Bevy | No current procedure selected | not-evaluated | Conversion, target mapping, runtime, and performance remain |

## Rig, masking, and compatibility evidence

The current 0.10.0 constituent rerun did not produce a collection compatibility matrix, rig comparison, mask trial, retarget trial, or cross-pack transition trial. No current conclusion about shared skeletons, masks, props, root authority, contacts, or co-installation follows.

A retained dated shared-path observation compared every one of the 28 unordered included-constituent pairs at 25 paths each: 700 byte comparisons were identical and no path conflicts were recorded. This is source-file overlap evidence only; it was not rerun and does not establish current runtime compatibility.

## Limitations and unknowns

- The report evaluates exactly eight constituents, not the full collection, and does not assess the fifteen excluded constituents.
- Canonical roles, logical-motion totals, runtime sets, cross-pack handoffs, and collection-wide remedial outputs are absent in the current run.
- No current engine import, playback, visual, target-character, retarget, contact, root-motion controller, compression, build, or performance evidence exists.
- Current external candidates do not establish source modification, promotion, derivative delivery, license rights, or artistic acceptance.

## Changes between AnimSmith versions

AnimSmith 0.10.0 — official release revalidated the eight constituent baselines and declared contracts; this appendix preserves their aggregate boundary without recreating historical collection taxonomy or runtime records.

AnimSmith 0.7.0 — the historical collection manifest, role taxonomy, runtime sets, cross-pack candidates, and collection remediation outputs are superseded. They are not current evidence.

AnimSmith 0.4.0 — the shared-path comparison and Unity probes are retained dated observations only.

## Reproduction

The current constituent evidence used the official Linux AnimSmith 0.10.0 release binary, SHA-256 `2052ce64eda53d5037b305561dd0287209719d743b0a4051552e197fbfe4a387`, from tag `v0.10.0` (peeled source commit `db91d8dda3326f97f581d4d62104d928caec383f`). Eight external, scrubbed constituent source inventories and their command manifests are linked from the paired constituent appendices.

No fresh collection lint output is cited: no current collection binding was rendered or accepted. Reproduce a future collection claim in a fresh external evidence root, with the official binary, declared source inventory, and only the project-relevant cross-pack, engine, and visual controls.

## Sources

- Constituent evidence appendices: [Basic Locomotion](protofactor-basic-locomotion-evidence.md), [Sword & Shield](protofactor-sword-and-shield-evidence.md), [Campfire](protofactor-campfire-evidence.md), [Climbing](protofactor-climbing-evidence.md), [Injured](protofactor-injured-evidence.md), [1-Handed Melee](protofactor-one-handed-melee-evidence.md), [2-Handed Melee](protofactor-two-handed-melee-evidence.md), and [Dual Swords](protofactor-dual-swords-evidence.md).
- Protofactor, [Ultimate Animation Collection](https://protofactor.biz/product/ultimate-animation-collection/) and [End User License Agreement](https://protofactor.biz/end-user-license-agreement/).
- AnimSmith, [game-ready clips](../game-ready-clips.md) and [CLI reference](../cli.md).
