# Animation pack evidence appendix: Mixamo Female Basic Locomotion

> Companion report: [Technical report](mixamo-female-basic-locomotion.md)
>
...
| Field | Value |
|---|---|
| Pack/edition | Mixamo Female Basic Locomotion; local revision unknown |
| Vendor/source | Vendor identity observed in delivered metadata; listing URL not retained |
| Delivered scope | 20 extracted FBX files: 7 in-place-directory and 13 root-motion-directory files |
...
| Evaluation manifest | `mixamo-female-basic-locomotion.manifest.json`; SHA-256 `aa732d482af4e42d97579c2493084ddc1a20151d03ce22f54e8c73ae6d7cae68`; legacy schema `urn:animsmith:skill:animation-pack-evaluation-manifest:1` |
...
The legacy metadata count of 18 motions is not linked authoritatively to these 20 files. Current conclusions use exact source paths and fresh measurements only.

### Evidence coverage

| Surface | Offered/delivered | Evaluated | Findings | Not evaluated and why |
|---|---:|---:|---:|---|
| Animation/source files | 20 | 20 | 0 baseline errors; 5 warnings | Vendor motion mapping unavailable |
| Rigs/export variants | 2 skeleton counts | 20 | two 68-bone `X Bot.fbx`; 18 66-bone files; Mixamo roles resolved | Retarget/deformation unavailable |
| AnimSmith baseline | 20 | 20 | 5 warnings; 2843 notes | — |
| Declared contracts | 20 | 20 | 6 ownership errors; 5 warnings | Per-file project intent unavailable |
...
| `other-unknown` | 20 | 20 | One opaque current source unit per delivered FBX |
| **Total** | **20** | **20** | Current exact source paths; no vendor mapping inferred |

### Exact runtime members

| Set/profile | Role or coordinate | Exact members | Variant/type | Timing or motion | Runtime contract |
|---|---|---|---|---|---|
| `hypothesis/kinematic-speed` | walk | `Female_Basic_Locomotion_Pack_-_in-place/walking.fbx` | variant=in-place | duration=0.967 s; frames=30 frames | loop=unknown; sync=not-evaluated; movement=controller-xz-yaw; playback=project-defined |
| `hypothesis/kinematic-speed` | run | `Female_Basic_Locomotion_Pack_-_in-place/running.fbx` | variant=in-place | duration=0.700 s; frames=22 frames | loop=unknown; sync=not-evaluated; movement=controller-xz-yaw; playback=project-defined |
...
|---|---|---|---|---|
| `hypothesis/kinematic-speed` | speed-blend | `Female_Basic_Locomotion_Pack_-_in-place/walking.fbx`; `Female_Basic_Locomotion_Pack_-_in-place/running.fbx`; all `in-place` | new evaluator hypothesis from current source names; no vendor/runtime authority; exact durations and frames are in the primary table | `not-evaluated` runtime acceptance |
...
| Inspect | `evaluated-clean` | All 20 FBXs admitted and resolved the Mixamo profile |
...
|---|---|---|---|
| Opaque delivered corpus | 0 errors; 5 warnings; all sources readable | Mixamo roles resolved; vendor semantics and retarget use unavailable | No current engine, visual, contact, or gameplay acceptance |
| `hypothesis/kinematic-speed` | Exact members and gait phases measured; loop intent unknown | New evaluator hypothesis; controller ownership proposed | Target-engine blend, phase/contact acceptance, transition, and deformation test required |
...
| Root-motion controller | `selected` — `observed-pack-capability` | 6 archive-level ownership conflicts require per-file policy |
...
The fresh inventory contains 20 FBX files. Both directory variants include `X Bot.fbx` with a 68-bone skeleton and an empty `Take 001`; the 18 motion-named files use 66 bones. All sources resolved the same nine Mixamo roles. This establishes static structure, not identical hierarchy/rest pose or retarget compatibility. Constant-track findings are optimization hints only.

The current scenario is intended to be useful without claiming old authority: `hypothesis/kinematic-speed` names 2 exact in-place-directory files, records current durations/frame counts, assigns movement to a hypothetical kinematic controller, and leaves loop intent, gait-phase acceptance, transitions, and runtime acceptance open. AnimSmith measured phase (cycles) as walk `0.4913` and run `0.4289`; the normalized circular-range spread is `0.0624` cycles, defined as `1 - largest cyclic gap` across member phases on the unit cycle. These are `observed-animsmith` measurements; `sync=not-evaluated`, and no contact or synchronization acceptance follows from the spread.

## Mechanical baseline

| Finding/check | Affected scope | Potential impact | Evidence |
|---|---|---|---|
| No baseline errors | 20/20 files | File-readable mechanical prerequisite only | `observed-animsmith`; current ledger |
| `duration-sanity` | 5 findings in 5 files | Empty reference take or shorter channels clamp-held at clip end | `Female_Basic_Locomotion_Pack_-_in-place/X Bot.fbx`; `Female_Basic_Locomotion_Pack_-_in-place/left strafe walk.fbx`; `Female_Basic_Locomotion_Pack_-_root-motion/X Bot.fbx`; `Female_Basic_Locomotion_Pack_-_root-motion/left strafe walk.fbx`; `Female_Basic_Locomotion_Pack_-_root-motion/left turn (2).fbx` |
| `constant-track` | 2843 notes | Possible storage/evaluation overhead; no visual defect or safe rewrite inferred | `observed-animsmith` |
| Mixamo role resolution | 20/20 files | Enables measurement; does not prove target-character retargeting | `observed-animsmith` |

## AnimSmith remediation evidence

| Source issue | Operation/declarations | Result | Independent verification | Remaining caveat |
|---|---|---|---|---|
| MIX-OWN-001 | Archive-level in-place and animation-owned XZ declarations | 6 stationary-root errors on `Female_Basic_Locomotion_Pack_-_root-motion/idle.fbx`; `Female_Basic_Locomotion_Pack_-_root-motion/jump.fbx`; `Female_Basic_Locomotion_Pack_-_root-motion/left turn (2).fbx`; `Female_Basic_Locomotion_Pack_-_root-motion/left turn.fbx`; `Female_Basic_Locomotion_Pack_-_root-motion/right turn (2).fbx`; `Female_Basic_Locomotion_Pack_-_root-motion/right turn.fbx` | Current per-file lint and measurement; no output produced | Per-file intent and controller acceptance unavailable; residual unresolved |
...
|---|---|---|---|---|---|
| `hypothesis/kinematic-speed` / target character | Mixamo roles resolve; target deformation not tested | Not evaluated | Controller XZ/yaw/collision proposed | Exact durations and gait phases measured; synchronization/blend acceptance not tested | `not-evaluated` beyond static source evidence |
...
1. Vendor mapping from 18 metadata motions to 20 files is unavailable and was not reconstructed from names.
...
AnimSmith 0.14.0 — Fresh current-source matrix admitted 20/20 FBXs and revalidated baseline and archive-level declarations. New controller topology is explicitly evaluator-proposed and remains runtime-unaccepted.
