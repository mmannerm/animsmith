...
>
> Evidence status: **partial** — exhaustive 0.4.0 baseline/contracts/remediation on one frozen evaluator (default and explicit-role passes), retained Unity 6000.5.8f1 eight-pack evidence, and new unity-humanoid/unreal/godot/bevy import-advice/addressability probes; transformed-clip visual acceptance and three engine editors/runtimes remain unevaluated.
>
...
| Unity `unity-humanoid` (new, 2026-08-21) | rev 1 / 6000.3 | `generate import-advice` on a delivered dominant-rig FBX under the `unity-humanoid` profile | Available, exit 0. Resolved settings are DERIVED, not observed: every delivered `.fbx.meta` has `useFileUnits: 1` and no `lockRootRotation`/`lockRootHeightY`/`lockRootPositionXZ` keys, so Unity's own absent-key default (`false`) implies `root_rotation`/`root_position_y`/`root_position_xz` = `extract`. This is a stated assumption about 6000.3 import-advice, not observed 6000.5.8f1 importer behavior | Confirm the derived settings against the actual project `.meta` files and observed 6000.5.8f1 import behavior |
...
The Unity 6000.5.8f1 probe is headless and retained unchanged from 2026-08-17, justified by the byte-identical re-verified source; it establishes import and graph execution only, not visible playback quality, contacts, foot plants, two-hand alignment, mask usefulness, retarget deformation, or shipping-build behavior. The 2026-08-21 import-advice/addressability rows are AnimSmith-side capability queries against profile revision 1 tuples: they show what the tool predicts or declines to model, not an actual Unreal/Godot editor import or a Bevy runtime load. Neither the historical `674396f` gait GLBs nor the new 0.4.0 gait GLBs were imported into the Unity project (no GLB importer configured there).
...
3. Unity testing was headless; neither the historical `674396f` gait-anchor GLBs nor the new 0.4.0 GLBs were imported, because the evaluation project has no GLB importer. Complete graphs, root extraction, contacts, compression, and builds remain open.
