...
|---|---|---|---|
| Unity 6000.3 / 6000.5.8f1 | Retained headless import/sampling (6000.5.8f1, 2026-08-17) plus `generate import-advice` under the frozen `unity-humanoid` rev 1 / 6000.3 / `fbx-model-importer` profile (2026-08-21). | 74/75 individual clips import as Humanoid; 5/5 required samples and a Basic-to-Climbing mixer pass; `FallingUnarmed` exposes no clip as expected (retained). Import-advice: `available` (exit 0) using declarations now **observed** in Unity 6000.5.8f1: in-place clips report all three lock-root flags `true` (bake) and root-motion clips report `lockRootPositionXZ` `false` (extract XZ). An earlier uniform `extract` reading, inferred from absent `.fbx.meta` keys, was wrong. | Visual contacts, in-Editor displacement/yaw acceptance, controller, target rig, compression, build; advice only, not observed behavior. |
