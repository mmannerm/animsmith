...
| Delivered scope | 83 extracted FBX files from paired archive variants |
...
| Evaluation manifest | `mixamo-rifle-8-way-locomotion.manifest.json`; SHA-256 `5f3a266b00f223047ec6ce5e0bef1e4e93c7d55dafd5bb4fc7ba26fee8e85dc4`; schema `urn:animsmith:skill:animation-pack-evaluation-manifest:1` |
| Acquisition/license provenance | No controlling license/readme/terms evidence was delivered; not legal advice |

The evaluation manifest schema is `urn:animsmith:skill:animation-pack-evaluation-manifest:1`. The validated manifest models every delivered FBX as an opaque `other-unknown` unit because vendor records were not linked to archive members; 81 remains separate source metadata.

### Evidence coverage

| Surface | Offered/delivered | Evaluated | Findings | Not evaluated and why |
|---|---:|---:|---:|---|
| Animation files | 83 | 83 | 11 baseline warnings | Semantic file-to-manifest mapping incomplete |
| Rigs/export variants | 1 | 1 | Mixamo profile used | Retarget behavior unavailable |
| AnimSmith baseline | 83 | 83 | 0 errors | — |
| Declared contracts | 83 | 83 | 16 errors; 8 warnings | Archive-level ownership only; per-clip intent unavailable |
| AnimSmith visual reports | 1 | 0 | 0 | One representative current HTML report generated externally but not visually inspected |
...
| `other-unknown` | 83 | 83 | One opaque unit per delivered FBX; no semantic classification asserted |
| **Total** | **83** | **83** | Validated scrubbed external manifest |
...
The external inventory contains 83 FBX files and 81 manifest-declared motions. Archive member counts exceed manifest motion counts by one non-motion/reference asset per archive. Delivered metadata declares 30 fps FBX export preferences, but this is not a target-engine import result.

## Mechanical baseline

| Finding/check | Affected scope | Potential impact | Evidence |
|---|---|---|---|
| No mechanical errors | 83 files | File-ready mechanical health only | observed-animsmith |
| `duration-sanity` warnings | 11 files | No-track assets/clips require semantic classification | observed-animsmith |
...
AnimSmith 0.10.0 refresh — all 83 baseline files and both archive-variant contracts were rerun. The retained 0.7.0 baseline had the same 11 warnings and 16 stationary-root findings; the current root-motion contract also records 8 `duration-sanity` warnings.
