...
This is a partial rollup of nine observed constituents. It does not establish cross-pack compatibility.
AnimSmith 0.7.0 mechanically inspected 249 extracted FBX files with no errors in the untouched baseline. The baseline retained 50 warnings and constant-track notes; these are not gameplay or engine acceptance. Variant-level in-place ownership was clean, while a blanket root-motion ownership declaration produced 52 stationary-root findings. No source bytes were changed. [The readiness ladder](../game-ready-clips.md#the-readiness-ladder) remains the governing boundary.
...
Cross-pack compatibility is unknown: no overlapping-path comparison, skeleton equivalence test, blend, retarget, or visual test was performed.

## Evidence status

249 delivered FBX files and 231 manifest-declared motions were evaluated mechanically with AnimSmith 0.7.0 at revision `461ac8a`. The retained external manifests use scrubbed SHA-256 identities only. The companion appendix records the schema and open evidence gates.
