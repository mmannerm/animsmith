...
**Other delivered motion leads, filename only:** `Standing Jump.fbx`, `Standing Land To Standing Idle.fbx`, `Standing Turn Left 90.fbx`, and `Standing Turn Right 90.fbx`. These names occur in the inspected source or issue inventory, but their intended controller roles and gameplay behavior have not been classified or tested. Inspect each source and its boundaries before adding a state; see the [retained source evidence](mixamo-magic-locomotion-evidence.md#animsmith-remediation-evidence). A zero canonical role count does not establish that these motions are absent.
