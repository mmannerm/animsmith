...
>
> Evaluation completeness: **partial** — all 27 extracted FBX files have current mechanical evidence; vendor mapping, runtime, visual, contact, retarget, license, and cross-pack acceptance remain unavailable.
>
> Confidence: **medium**
>
...
For a prototype, start with four-direction full-body magic running using the selected files in the [runtime set](#runtime-sets-and-authored-motion). Let the kinematic controller own XZ movement, yaw, and collision; the selected roots are effectively stationary. This is an evaluator-proposed set (`hypothesis/kinematic-run-4way`), with no accepted loop, blend, foot-contact, or target-engine result. Keep it out of production until the [issue gates](#technical-issue-register) and [engine review](#engine-status) pass.

All 27 delivered FBX files loaded under AnimSmith 0.14.0 and the untouched baseline produced 0 errors, 6 `duration-sanity` warnings, and 3837 non-gating `constant-track` notes. Applying one animation-owned movement rule across the archive produced 4 errors where files in the root-motion directory remain effectively stationary. Decide movement ownership per file before using those states; a directory name alone does not establish authored displacement. Two `X Bot.fbx` files use a 68-bone reference skeleton; the other 25 files use 66 bones, and every file resolves the nine-role Mixamo profile.
...
|---|---|
| Four standing run directions are named and measured. Standing jump, land, idle and turn files appear by filename; their controller roles are unclassified and untested. | Check the named states and their transitions in the target game before treating this as a complete controller. |

### Content gaps and unknowns

The legacy count of 25 motions has no verified mapping to these 27 files. Classify the other named motions before adding them to a controller.
...
Best fit is full-body magic locomotion prototyping where a developer can own movement, define thresholds, and test the exact target character. It is a poor fit for drop-in root-motion, network, motion-matching, layered-animation, or production-ready claims.
...
AnimSmith 0.14.0 — The current evaluation reran 996 inspect, measure, and lint commands across the collection. This constituent admitted 27/27 files; baseline and declared-contract findings were revalidated, and the selected controller set was proposed from current files. Its clip roles remain unverified.
...
The current tests covered 27 FBX source files individually. The legacy metadata lists 25 motions, but its clip-to-file mapping is unverified for the 27 files. The [evidence appendix](mixamo-magic-locomotion-evidence.md) records the source integrity check, evaluator revision, command results, and digests. See the [canonical readiness ladder](../game-ready-clips.md#the-readiness-ladder).
