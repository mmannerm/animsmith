...
AnimSmith 0.10.0 mechanically inspected all 27 extracted FBX files with no errors in the untouched baseline. The baseline retained 6 `duration-sanity` warnings and 3,837 `constant-track` notes; these are not gameplay or engine acceptance. The archive-level in-place contract produced no errors, while the archive-level root-motion contract produced 4 stationary-root errors and 5 `duration-sanity` warnings. No source bytes were changed. [The readiness ladder](../game-ready-clips.md#the-readiness-ladder) remains the governing boundary.
