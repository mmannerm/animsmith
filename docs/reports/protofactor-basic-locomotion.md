...
| BL-PHASE | major | Walk/run/crouch direction sets have phase spread 0.6598/0.4630/0.7156 cycles; mismatched support feet can produce foot sliding when blended. [Readiness guidance](../game-ready-clips.md#the-readiness-ladder). | artist-author | Align authored cycles to a common foot phase. Current anchored spreads are 0.0724/0.0938/0.0502 cycles in the same set order. Review 24 gait-anchor candidates; 22 still fail declared lint. Phase disagreement is mechanically reduced in candidates; residual adoption remains unresolved while loop/contact gates remain open. No output promoted. | Current gait anchoring helps a measured component; contact and loop corrections still need validation. | Exact sets and source/output phase measurements; `observed-animsmith`, gameplay effect `inferred`. |
