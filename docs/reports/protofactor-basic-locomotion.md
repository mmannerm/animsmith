...
| AP-009 | Moderate | Combined FBX, bundled animation list, and Unity clip range disagree; unsliced take is not gameplay-ready | vendor-license | Use the 177 per-motion files; only ask an artist/author to segment the combined take when authoritative boundaries are supplied | Declared slicing can execute known ranges, but cannot invent missing semantic boundaries | High file evidence |
