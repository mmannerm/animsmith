...
>
> Evaluation completeness: **partial** — complete 0.7.0 baseline, contracts, remediation, addressability, and bounded advice plus a dated Unity probe; no visual controller, target character, or current engine pass.
>
> Confidence: **medium**
>
...
AnimSmith 0.7.0 verifies the unchanged 113-FBX manifest and runs the baseline, contracts, and gait remediation on one evaluator. Gait anchoring produces 24 in-place candidates, reducing walk/run/crouch phase spreads from 0.554/0.734/0.714 to 0.064/0.108/0.039. The candidates stay unpromoted because no Humanoid retarget or visual import ran. Constant-track pruning produces one additional unaccepted candidate.
...
| OH-002 | major | [Incorrect loop declarations](../game-ready-clips.md#the-loop-pops) mark 30 obvious attacks, combos, hits, and recoveries as loops, risking repeated actions and hard wraps. | engine-config | Override one-shot-like flags and review remaining loop candidates. | A metadata/role-aware audit is feasible; universal intent inference is not. | 93/110 loop flags; 87 declared-loop contract failures; 37 no-stride/stationary clips are `not_evaluated`. |
| OH-003 | major | [Gait-phase disagreement](../game-ready-clips.md#feet-skate-when-clips-blend) risks foot skating. | animsmith-current-declared | Trial the 24 IP candidates, then validate and offset residual phase; keep RM raw. | Current IP heading support produces candidates; trajectory-preserving RM remains untested. | Phase spread is materially lower; engine and visual acceptance remain open. |
| OH-004 | major | [Directional RM speed variation](../game-ready-clips.md#directional-blend-members-travel-at-different-speeds) can make input magnitude change with direction. | engine-config | Preserve velocity per member or tune playback against controller policy. | Current declared-set policies can check a supplied controller contract; this evaluation did not supply one. | Walk 1.94×; crouch 1.52×; run 1.11×. |
| OH-005 | moderate | [RM action ownership](../game-ready-clips.md#the-character-glides-or-runs-in-place) is not established by `_RM` or horizontal speed alone, risking doubled or missing displacement/yaw. | engine-config | Review the current root-trajectory measurements; never infer ownership from measured travel. | Per-clip displacement/yaw is available; continuous-curve extraction proof is not. | 112/112 measured: 39 moving, 72 stationary, 0 yawing; `heading_axis`=`positive_y` on 111/111 — sampled facts, not extraction proof. |
| OH-006 | moderate | [Dense constant tracks](../game-ready-clips.md#the-file-is-bloated-or-the-retargeter-chokes) may waste memory; unproved pruning can change sparse-track behavior. | animsmith-current-declared | Keep sources until runtime/equivalence gates pass. | Per-clip channel coverage is available; [#401](https://github.com/mmannerm/animsmith/issues/401) limits broad pruning adoption. | 13,360 contract notes; one export candidate (`Humanoid@IdleCombat1hMelee.fbx`), source unmodified. |

## Engine status

| Runtime | Evidence level | Technical result | Remaining gate |
|---|---|---|---|
| Unity | Observed eight-pack co-import plus 0.7.0 advice | **Conditional pass:** 108/110 clips are Humanoid; six samples, two mixers, one mask, and one prop test pass. Advice matches observed locks: IP bakes, while 31/36 RM clips extract XZ. | Visual controller, contacts, grip, root motion, retargeting, compression, build. |
| Unreal Engine | Import-advice | **Not evaluated in-engine.** Current revision-2 settings projection is available; no engine process ran. | FBX import, retarget, graphs, contacts, build. |
| Godot | Import-advice | **Not evaluated in-engine.** Current revision-2 settings projection is available; no engine process ran. | Import/conversion, retarget, graphs, contacts, export. |
...
AnimSmith 0.4.0 consolidated the baseline, contract, and successful 24-member gait result that the current evaluator revalidates. The pre-release evaluator produced the same phase figures but is no longer the current authority.

## Evidence status

The [readiness ladder](../game-ready-clips.md#the-readiness-ladder) and [appendix](protofactor-one-handed-melee-evidence.md) define the current evidence boundary. Local revision and terms remain unproven.
