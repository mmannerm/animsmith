...
The current baseline is mechanically readable with AnimSmith 0.10.0, but this pack is not a blanket game-ready asset. 452 baseline commands completed successfully; constant-track notes and strict loop contracts require review. 25 bounded remediation candidates transformed successfully; they remain candidates pending engine, contact, and visual review.
...
- 113 delivered FBX candidates were inspected, measured, and linted in both output formats.
- Declared contracts and bounded remediation trials were rerun with the current evaluator.

### Partial supporting gameplay

- 110 declared-contract inputs: 23 clean and 87 non-clean results in each output format.
...
See the [readiness ladder](../game-ready-clips.md#the-readiness-ladder) and [appendix](protofactor-one-handed-melee-evidence.md). Mechanical completion is separate from engine and visual acceptance.
