...
- Mechanical intake covers 123 FBXs, 122 animation-bearing files, 120 individual motions, and 1 combined take.
...
Measured RM speed spans are `crouch-combat-8-way` 0.642–0.835 m/s (ratio 0.769); `run-combat-8-way` 2.202–2.690 m/s (ratio 0.819); `walk-combat-8-way` 0.810–1.092 m/s (ratio 0.742). Preserve authored variation unless the project declares a normalization policy; diagonals and cardinals require the full blend test. Current in-place gait-phase spreads are large enough that `sync=not-evaluated` remains the correct contract. The current catalog also measures `normal-forward-speed`, `draw-combat-put-away`, `dodge-forward-back`, and `parry-3-way` as evaluator-defined hypotheses; they remain candidate groupings.
