...
The external inventory contains 29 FBX files and 27 manifest-declared motions. Archive member counts exceed manifest motion counts by one non-motion/reference asset per archive. Delivered metadata declares 30 fps FBX 2019 export preferences, but this is not a target-engine import result.
