...
| Evaluator scenario | 1 | 1 static proposal | 3 members with measured gait phase | Loop, synchronization, contact, engine, and visual acceptance unavailable |
...
| `hypothesis/kinematic-walk-3way` | Exact members and gait phases measured; loop intent unknown | New evaluator hypothesis; controller ownership proposed | Target-engine blend, phase/contact acceptance, transition, and deformation test required |
...
The current scenario is intended to be useful without claiming old authority: `hypothesis/kinematic-walk-3way` names 3 exact in-place-directory files, records current durations/frame counts, assigns movement to a hypothetical kinematic controller, and leaves loop intent, gait-phase acceptance, transitions, and runtime acceptance open. AnimSmith measured phase (cycles) as forward `0.4841`, left `0.4905`, and right `0.4587`; the normalized circular-range spread is `0.0318` cycles, defined as `1 - largest cyclic gap` across member phases on the unit cycle. These are `observed-animsmith` measurements; `sync=not-evaluated`, and no contact or synchronization acceptance follows from the spread.
...
|---|---|---|---|---|---|
| `hypothesis/kinematic-walk-3way` / target character | Mixamo roles resolve; target deformation not tested | Not evaluated | Controller XZ/yaw/collision proposed | Exact durations and gait phases measured; synchronization/blend acceptance not tested | `not-evaluated` beyond static source evidence |
