...
| Delivered scope | 12 extracted FBX files from paired archive variants |
...
| Evaluation manifest | `mixamo-basic-locomotion.manifest.json`; SHA-256 `65c359befba9cb1a73539bf0af73e575a4d1e9cf5e2085152940af7648ec94b0`; schema `urn:animsmith:skill:animation-pack-evaluation-manifest:1` |
| Acquisition/license provenance | No controlling license/readme/terms evidence was delivered; not legal advice |

The evaluation manifest schema is `urn:animsmith:skill:animation-pack-evaluation-manifest:1`. The validated manifest models every delivered FBX as an opaque `other-unknown` unit because vendor records were not linked to archive members; 10 remains separate source metadata.

### Evidence coverage

| Surface | Offered/delivered | Evaluated | Findings | Not evaluated and why |
|---|---:|---:|---:|---|
| Animation files | 12 | 12 | 0 errors; 4 baseline warnings | Semantic file-to-manifest mapping incomplete |
| Rigs/export variants | 1 | 1 | Mixamo profile used | Retarget behavior unavailable |
| AnimSmith baseline | 12 | 12 | 0 errors; 4 warnings; 1,712 notes | — |
| Declared contracts | 12 | 12 | 4 root-motion stationary-root errors; 1 in-place and 3 root-motion duration warnings | Per-clip intent unavailable |
...
| `other-unknown` | 12 | 12 | One opaque unit per delivered FBX; no semantic classification asserted |
| **Total** | **12** | **12** | Validated scrubbed external manifest |
...
| Root motion | `partially-evaluated` | In-place XZ contract has 0 errors and 1 duration warning; root-motion XZ contract has 4 stationary-root errors and 3 duration warnings; per-clip ownership remains open |
...
The external inventory contains 12 FBX files and 10 manifest-declared motions. Archive member counts exceed manifest motion counts by one non-motion/reference asset per archive. Delivered metadata declares 30 fps FBX 2019 export preferences, but this is not a target-engine import result.

## Mechanical baseline

| Finding/check | Affected scope | Potential impact | Evidence |
|---|---|---|---|
| No mechanical errors | 12 files | File-ready mechanical health only | observed-animsmith, AnimSmith 0.10.0 |
...
AnimSmith 0.10.0 — Official `v0.10.0` rerun revalidated output schema v19 and measurements schema v18: 12/12 FBX admitted, baseline 0 errors, and the root-motion declaration found 4 stationary-root errors. AnimSmith 0.7.0 (output schema v17; measurements schema v16) is retained historical evidence only.
