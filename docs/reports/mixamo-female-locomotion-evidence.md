...
| Evaluation manifest | `mixamo-female-locomotion.manifest.json`; SHA-256 `4f0df4fda4c22806c29378f5d7b7a62cd88abc90d03324c16dc3cfdf0b0a3906`; legacy schema `urn:animsmith:skill:animation-pack-evaluation-manifest:1` |
...
|---|---:|---:|---:|---|
| Animation/source files | 18 | 18 | 0 baseline errors; 4 warnings | Vendor motion mapping unavailable |
...
| AnimSmith baseline | 18 | 18 | 4 warnings; 2559 notes | — |
| Declared contracts | 18 | 18 | 4 ownership errors; 4 warnings | Per-file project intent unavailable |
...
|---|---|---|---|---|
| `hypothesis/kinematic-speed` | speed-blend | `Female_Locomotion_Pack_-_in-place/walking.fbx`; `Female_Locomotion_Pack_-_in-place/running.fbx`; all `in-place` | new evaluator hypothesis from current source names; no vendor/runtime authority; exact durations and frames are in the primary table | `not-evaluated` runtime acceptance |
...
| `duration-sanity` | 4 findings in 4 files | Empty reference take or shorter channels clamp-held at clip end | `Female_Locomotion_Pack_-_in-place/X Bot.fbx`; `Female_Locomotion_Pack_-_in-place/left strafe walk.fbx`; `Female_Locomotion_Pack_-_root-motion/X Bot.fbx`; `Female_Locomotion_Pack_-_root-motion/left strafe walk.fbx` |
| `constant-track` | 2559 notes | Possible storage/evaluation overhead; no visual defect or safe rewrite inferred | `observed-animsmith` |
