...
| `[clips."<name>"] loop`, `max_loop_position_delta_m` and its three sibling caps | That the clip is a cycle, and how far its endpoints may drift | Artist alone for `loop`; the caps are a project bar set together | The take was authored as a cycle; the caps start at the defaults | `[clips.walk]`<br>`loop = true` |
...
| `[clips."<name>"] movement_owner_xz`, `movement_owner_y`, `movement_owner_yaw` | Whether gameplay or the animation moves the character on each axis | Game developer alone — a character-controller decision | The engine project: whichever is actually driving movement | `[clips.walk]`<br>`movement_owner_xz = "gameplay"` |
...
| `[gait_groups."<name>"]`, `[sync_groups."<name>"]` | Which clips blend as one ring, and which play at the same time | Together — the technical animator declares the ring and its phase tolerance, the game developer confirms the runtime blend set | The blend graph or state machine that will crossfade them | `[gait_groups.walk-ring]`<br>`clips = ["walk_f", "walk_b"]`<br>`max_gait_phase_spread = 0.15` |
...
| `[engine]`, `[engine.settings]` | Which importer boundary an engine prediction is about; all four selection keys are required together | Game developer alone — the shipping project's own tuple | The engine project: version, importer, import settings | `[engine]`<br>`profile = "bevy"`<br>`profile_revision = 3`<br>`engine_version = "0.19.0"`<br>`importer = "gltf-asset-loader"`<br>`[engine.settings]`<br>`extension_handler_environment = "bare_empty"`<br>`bevy_animation_feature = true`<br>`load_animations = true` |
...
| Collection manifest, `[transition_families]` | Which files hold which clips, and which takes form a family | Together — the pack owner states what the delivery contains, the game developer binds it to runtime sets | The delivery itself; see [collection contracts](collection-contracts.md) | See [collection contracts](collection-contracts.md#transition-families-148) |
