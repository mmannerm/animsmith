...
| [The file is bloated, or the retargeter chokes](file-bloat.md) | `constant-track`, `scale-keys`, `non-uniform-scale`, `rest-world-scale` | `transform --prune-constant-tracks` after reviewing transition coverage | `[checks.<id>] severity`, `[runtime_nodes] selectors` | Artist or exporter settings |
