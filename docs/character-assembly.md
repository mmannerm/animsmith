...
a second recipe namespace. When `rest_bind_scale` is enabled, an explicit
selection that retains no skinned base instance refuses as an
`asset-recipe-mismatch` at the selection boundary; its diagnostic names the
recipe selection rather than the private staging artifact.
