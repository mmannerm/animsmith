...
independently validated `urn:animsmith:schema:collection-output:11` document.
...
identity, and retains a path-scrubbed typed row for every V11 source. The model
...
.agents/skills/evaluate-animation-packs/scripts/validate_evaluation_model_v2.py \
  MODEL.json --binding COLLECTION-V11.json \
  --animsmith /exact/checkout/target/release/animsmith --check-canonical
```

The offline registry is pinned to collection-output V11, output V19, and
measurements V18. Wrong, historical, or future schema identities fail rather
...
exact internal success handshake naming collection-output V11. Python requires
...
and nested one-file lint payloads remain in the exact hashed V11 input rather
than being copied into the public model.

The model must contain every V11 clip and runtime set. Clip source, declared take
index, and exact take name must match. Established duration and root-speed
facts must equal V11 measurements. Root-speed availability is exact:
...
V11 slash-separated logical IDs remain valid everywhere they are referenced,
...
their original validator path. A V1 model plus V11 input is not a V2 evaluation;
