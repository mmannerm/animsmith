...
caps cover retained name bytes, candidate storage bytes, tracks, authored
keys/values, generated keys, and aggregate inspection work, with N+1 refusal
before candidate allocation. Candidate value storage needs no separate limit:
retained cubic storage is already bounded by authored values, while LINEAR/STEP
emit one value per bounded generated key. This slice does not prove that the
