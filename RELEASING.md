...
4. Review that PR. If step 2 staged nothing, or staged a version release-plz
   ...
   cargo test -p animsmith --test docs_visuals
   ```
   ...
   lock's size and SHA-256 in the generated module
   `crates/animsmith-engine/src/bevy_readback_lock.rs`. Move both package
   versions in `tools/bevy-readback/Cargo.lock` to the release, regenerate
   that module from the lock, and confirm the lock gate agrees:
   ...
   Do not hand-edit the generated module: `just bevy-readback-lock` renders it
   again from the committed lock and fails on any difference.
