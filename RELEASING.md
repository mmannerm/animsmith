...
4. Review that PR. If step 2 staged nothing, or staged a version release-plz
   ...
   version in generated bytes and pin the resulting SHA-256 values — whole-file
   digests, which do move with the version, unlike the `PAYLOAD_IDENTITIES`
   pins above. In particular, run the exact-artifact assembly test before the
   full gate:
   ...
   If a version-only bump changes those whole-file digests, capture the
   generated values before editing the assertions and verify that the
   source-byte difference is limited to expected version-bearing metadata (plus
   deterministic downstream digests). Refresh only the proven version-sensitive
   literals; do not accept a blanket golden update. That refresh never reaches
   the `PAYLOAD_IDENTITIES` pins, which are version-independent by
   construction.
