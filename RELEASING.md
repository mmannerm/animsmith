...
4. Review that PR. If step 2 staged nothing, or staged a version release-plz
   ...
   release version. The example assets, documentation visuals, and Bevy lock
   are the three mechanical regenerations this step retains. Payload identities
   protect the example assets and exact-artifact assembly fixture independently
   of the package version; the visuals and Bevy lock/identity remain exact
   regenerated artifacts checked by their own gates. There are no manual
   assembly digest literals to refresh and no counterfactual assembly rebuild
   to perform.
